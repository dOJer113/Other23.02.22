    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < products.length; i++) {
            if (counts[i] != 0 && doBonus==true) {
                ...
                if (isOnSale) {
                    System.out.println("\t" + products[i] + " " + counts[i] + " шт. за " + (prices[i] * ((counts[i]-1) / 3 * 2 + (counts[i]-1) % 3)) + " руб. (распродажа!)");
                    sum += prices[i] * ((counts[i]-1) / 3 * 2 + (counts[i]-1) % 3);
                } else {
                    System.out.println("\t" + products[i] + " " + counts[i] + " шт. за " + (prices[i] * (counts[i]-1)) + " руб.");
                    sum += prices[i] * (counts[i]-1);
                }
            }
            else if(counts[i]!=0){
                sum = 0;
